// API handler for /api/intents/[id]
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      if (result.success) {
        return res.status(200).json({ success: true, message: 'Intent deleted successfully' });
      } else {
    ...
    console.error(`Error in Intent API handler:`, error);
